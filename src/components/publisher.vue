<script setup>
import { ref } from 'vue';
import { SearchInput, PublisherItem } from '.'
...
<template>
    <div v-if="props.data.length > 0"
        ...
        <div class="flex">
            ...
            <div v-if="!searchInput" @click="showSearchInput(true)" class="w-full">
        ...
        <ul class="divide-y divide-gray-200 border dark:divide-gray-700 bg-gray-100 rounded-md dark:border-gray-600">
            <PublisherItem v-for="item in props.data" :key="item._id" :item="item" />
        </ul>
    </div>
</template>